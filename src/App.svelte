...
<main class="w-full h-full flex flex-col dark:bg-black bg-white">
    <div class="w-full h-screen flex items-center flex-col p-4">
        ...
        <div class="relative flex mt-[5vh] w-3/4">
            <Editor id="mainEditor" minHeight="40vh" lines langSelector lineSelector bind:value={code} lineSelectorClass="bg-gray-900/50 hover:bg-gray-900/60" dots langSelectorClass="bg-gray-900/50 hover:bg-gray-900/60" language="shell" maxHeight="40vh" lineNumberClass="!border-0 bg-gray-900/30 rounded-md" defaultTextClass="text-gray-200/50 text-sm ml-4" caretColor="rgba(255, 255, 255, 0.5)" class="absolute w-full !ml-0 !top-0 !left-0 text-gray-200/50 bg-gray-950 backdrop-blur z-50 !m-8">
            </Editor>
            <div id="gradient" style="height:40vh;" class="mt-8 h-full absolute bg-gradient-to-r from-orange-500 to-purple-500 rounded-lg blur-sm w-full z-0"></div>
        </div>
    </div>
</main>
